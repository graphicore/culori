	...
	var culori = (() => {
		...
		var useParser = (parser, mode) => {
			if (typeof parser === 'string') {
				if (!mode) {
					throw new Error(
						`'mode' required when 'parser' is a string`
					);
				}
		...
		function parseColorSyntax(tokens) {
			...
			for (let ii = 0, c4; ii < channels.length; ii++) {
				...
				if (c4.type !== Tok.None) {
					res[channels[ii]] =
						c4.type === Tok.Number ? c4.value : c4.value / 100;
				}
			}
			return res;
		}
		...
		function parseRgb(color, parsed) {
			...
			if (r2.type !== Tok.None) {
				res.r =
					r2.type === Tok.Number ? r2.value / 255 : r2.value / 100;
			}
			if (g.type !== Tok.None) {
		...
		var fn2 = c4 => {
			...
			if (abs3 > 31308e-7) {
				return (
					(Math.sign(c4) || 1) *
					(1.055 * Math.pow(abs3, 1 / 2.4) - 0.055)
				);
			}
			return c4 * 12.92;
		};
		...
		var normalizeHue = hue3 =>
			(hue3 = hue3 % 360) < 0 ? hue3 + 360 : hue3;
		...
		var differenceCie94 = (kL = 1, K1 = 0.045, K2 = 0.015) => {
			...
			return (std, smp) => {
				...
				let dH2 =
					Math.pow(aStd - aSmp, 2) + Math.pow(bStd - bSmp, 2) - dC2;
				return Math.sqrt(
					dL2 / Math.pow(kL, 2) +
		...
		var differenceCiede2000 = (Kl = 1, Kc = 1, Kh = 1) => {
			...
			return (std, smp) => {
				...
				let deltaTheta =
					((30 * Math.PI) / 180) *
					Math.exp(
						-1 * Math.pow(((180 / Math.PI) * hp - 275) / 25, 2)
					);
				let Rc =
					2 *
					Math.sqrt(
		...
		var differenceCmc = (l = 1, c4 = 1) => {
			...
			return (std, smp) => {
				...
				let dH2 =
					Math.pow(aStd - aSmp, 2) + Math.pow(bStd - bSmp, 2) - dC2;
				let F = Math.sqrt(
					Math.pow(cStd, 4) / (Math.pow(cStd, 4) + 1900)
				);
				let T =
					hStd >= (164 / 180) * Math.PI &&
		...
		var convertLab65ToRgb = lab2 =>
			convertXyz65ToRgb_default(convertLab65ToXyz65_default(lab2));
		...
		var convertRgbToLab65 = rgb4 => {
			let res = convertXyz65ToLab65_default(
				convertRgbToXyz65_default(rgb4)
			);
		...
		var definition5 = {
			...
			toMode: {
				...
				rgb: c4 =>
					convertLab65ToRgb_default(convertDlchToLab65_default(c4))
			},
			fromMode: {
				lab65: convertLab65ToDlch_default,
				dlab: c4 => convertLabToLch_default(c4, 'dlch'),
				rgb: c4 =>
					convertLab65ToDlch_default(convertRgbToLab65_default(c4))
			},
		...
		var convertRgbToJab = rgb4 => {
			let res = convertXyz65ToJab_default(
				convertRgbToXyz65_default(rgb4)
			);
			if (rgb4.r === rgb4.b && rgb4.b === rgb4.g) {
				res.a = res.b = 0;
		...
		var fn4 = v =>
			Math.pow(v, 3) > e3 ? Math.pow(v, 3) : (116 * v - 16) / k3;
		...
		var convertLabToRgb = lab2 =>
			convertXyz50ToRgb_default(convertLabToXyz50_default(lab2));
		...
		var f2 = value =>
			value > e3 ? Math.cbrt(value) : (k3 * value + 16) / 116;
		...
		var convertRgbToLab = rgb4 => {
			let res = convertXyz50ToLab_default(
				convertRgbToXyz50_default(rgb4)
			);
			if (rgb4.r === rgb4.b && rgb4.b === rgb4.g) {
				res.a = res.b = 0;
			}
		...
		var definition15 = {
			...
			toMode: {
				...
				rgb: c4 =>
					convertLab65ToRgb_default(
						convertLchToLab_default(c4, 'lab65')
					)
			},
			fromMode: {
				rgb: c4 =>
					convertLabToLch_default(
						convertRgbToLab65_default(c4),
						'lch65'
					),
		...
		var convertRgbToLchuv = rgb4 =>
			convertLuvToLchuv_default(
				convertXyz50ToLuv_default(convertRgbToXyz50_default(rgb4))
			);
		...
		var convertLrgbToOklab = ({ r: r2, g, b, alpha }) => {
			...
			let M2 = Math.cbrt(
				0.2119034981999999 * r2 +
					0.6806995450999999 * g +
					0.1073969566 * b
			);
			let S = Math.cbrt(
				0.08830246189999998 * r2 +
					0.2817188376 * g +
					0.6299787005000002 * b
			);
			let res = {
				mode: 'oklab',
		...
		var convertRgbToOklab = rgb4 => {
			let res = convertLrgbToOklab_default(
				convertRgbToLrgb_default(rgb4)
			);
			if (rgb4.r === rgb4.b && rgb4.b === rgb4.g) {
				res.a = res.b = 0;
			}
		...
		function find_cusp(a, b) {
			...
			let rgb4 = convertOklabToLrgb_default({
				l: 1,
				a: S_cusp * a,
				b: S_cusp * b
			});
		...
		function convertOklabToOkhsl(lab2) {
			...
			if (s) {
				...
				ret.h = normalizeHue_default(
					(Math.atan2(lab2.b, lab2.a) * 180) / Math.PI
				);
			}
			return ret;
		}
		...
		var definition20 = {
			...
			toMode: {
				...
				rgb: c4 =>
					convertOklabToRgb_default(
						convertLchToLab_default(c4, 'oklab')
					)
			},
			fromMode: {
				rgb: c4 =>
					convertLabToLch_default(
						convertRgbToOklab_default(c4),
						'oklch'
					),
		...
		var definition22 = {
			...
			fromMode: {
				...
				rgb: color =>
					convertXyz50ToProphoto_default(
						convertRgbToXyz50_default(color)
					)
			},
			toMode: {
				...
				rgb: color =>
					convertXyz50ToRgb_default(
						convertProphotoToXyz50_default(color)
					)
			}
		};
		...
		var gamma3 = v => {
			...
			if (abs3 > β) {
				return (
					(Math.sign(v) || 1) * (α * Math.pow(abs3, 0.45) - (α - 1))
				);
			}
			return 4.5 * v;
		};
		...
		var linearize3 = v => {
			...
			return (
				(Math.sign(v) || 1) * Math.pow((abs3 + α2 - 1) / α2, 1 / 0.45)
			);
		};
		...
		var definition23 = {
			...
			fromMode: {
				...
				rgb: color =>
					convertXyz65ToRec2020_default(
						convertRgbToXyz65_default(color)
					)
			},
			toMode: {
				xyz65: convertRec2020ToXyz65_default,
				rgb: color =>
					convertXyz65ToRgb_default(
						convertRec2020ToXyz65_default(color)
					)
			},
			parse: ['rec2020'],
			serialize: 'rec2020'
						...
						if (ch !== 'alpha') {
							if (alpha === 0) {
								...
								res[ch] =
									...
									s.alpha * b.alpha * fn5(b[ch], s[ch]) +
									(1 - s.alpha) * b.alpha * b[ch];
								res[ch] = Math.max(
									0,
									Math.min(1, res[ch] / alpha)
								);
							}
						}
						return res;
		...
		var mapper = (fn5, mode = 'rgb', preserve_mode = false) => {
			...
			return color => {
				...
				let res = (
					channels || getMode(conv_color.mode).channels
				).reduce(
					(res2, ch) => {
		...
		var solve = v => {
			...
			for (i = 2; i < n3; ++i) {
				c4[i] = 1 / (4 - c4[i - 1]);
				_v[i] =
					(6 * v[i] - (i == n3 - 1 ? v[n3] : 0) - _v[i - 1]) * c4[i];
			}
			sol[0] = v[0];
			sol[n3] = v[n3];
			if (n3 - 1 > 0) {
		...
		var interpolatorSplineNatural = arr =>
		...
		var interpolator = (arr, yp, s) => {
			...
			return t => {
				...
				let t3 = t2 * t1;
				return (
					(yp[i] + yp[i + 1] - 2 * s[i]) * n22 * t3 +
					(3 * s[i] - 2 * yp[i] - yp[i + 1]) * n3 * t2 +
					yp[i] * t1 +
					arr[i]
				);
			};
		...
		var interpolatorSplineMonotone2 = arr => {
			...
			yp[0] =
				p4[0] * s[0] <= 0
					? 0
					: abs2(p4[0]) > 2 * abs2(s[0])
					? 2 * s[0]
					: p4[0];
			yp[n3] =
				p4[n3] * s[n3 - 1] <= 0
					? 0
					: abs2(p4[n3]) > 2 * abs2(s[n3 - 1])
					? 2 * s[n3 - 1]
					: p4[n3];
			return interpolator(arr, yp, s);
		};
		var interpolatorSplineMonotoneClosed = arr => {
			...
			yp[0] =
				(sgn(s_m1) + sgn(s[0])) *
				min(abs2(s_m1), abs2(s[0]), 0.5 * abs2(p4[0]));
			yp[n3] =
				(sgn(s[n3 - 1]) + sgn(s_n)) *
				min(abs2(s[n3 - 1]), abs2(s_n), 0.5 * abs2(p4[n3]));
			return interpolator(arr, yp, s);
		};
		...
		function inGamut(mode = 'rgb') {
			...
			const conv = converter_default(
				typeof gamut === 'string' ? gamut : mode
			);
		...
		function toGamut(
			...
			return color => {
				...
				return destConv(
					inDestinationGamut(candidate) ? candidate : clipped
				);
			};
		}
		...
		var matrix = (values, mode, preserve_mode = false) => {
			...
			return color => {
				...
				for (let i = 0; i < values.length; i++) {
					...
					res[ch] =
						(res[ch] || 0) +
						values[i] * (c4[channels[i % count]] || 0);
				}
				if (!preserve_mode) {
					return res;
				}
		...
		var filterDeficiencyDeuter = (severity = 1) =>
